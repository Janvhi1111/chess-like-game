class Game {
    ...
    // Method to get valid moves for the AI (Player 2)
getValidMoves(player) {
    const moves = [];
    for (let row = 0; row < 5; row++) {
        for (let col = 0; col < 5; col++) {
            if (this.board[row][col] && this.board[row][col].player === player) {
                // Example of simple move generation (1-step in all directions)
                if (row > 0) moves.push({ from: [row, col], to: [row - 1, col] });
                if (row < 4) moves.push({ from: [row, col], to: [row + 1, col] });
                if (col > 0) moves.push({ from: [row, col], to: [row, col - 1] });
                if (col < 4) moves.push({ from: [row, col], to: [row, col + 1] });
            }
        }
    }
    return moves;
...
// Method to make a random move for the AI
makeAIMove() {
    const validMoves = this.getValidMoves(2); // AI is Player 2
    if (validMoves.length > 0) {
        const move = validMoves[Math.floor(Math.random() * validMoves.length)]; // Choose a random move
        this.makeMove(2, move.from[0], move.from[1], move.to[0], move.to[1]);
    }
}
