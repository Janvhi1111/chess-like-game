...
function updateGame(board, player) {
    ...
    // If it's AI's turn in single-player mode, make AI move
    if (isSinglePlayer && currentPlayer === 2) {
        setTimeout(() => {
            game.makeAIMove(); // Call AI move function
            currentPlayer = 1; // Switch back to Player 1
            renderBoard();
            ws.send(JSON.stringify({ type: 'update', board: currentBoard, currentPlayer: 1 })); // Send updated board
        }, 500); // Delay for AI's move to simulate thinking time
    }
}
